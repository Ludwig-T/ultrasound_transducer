...
if os.path.isfile("ludwig_code/processed_data/results_quick2.npz"):
    file = np.load("ludwig_code/processed_data/results_quick2.npz")
    coord = file["coord"]
    z = file["z"]
else:
    ...
    for file in os.listdir(folder):
        ...
        z.append(np.std(selected_values))
        to_print = name + " " + coord_str[0] + " " + coord_str[1] + " {}"
        print(to_print.format(z[-1]))
    np.savez("ludwig_code/processed_data/results_quick2.npz", coord=coord, z=z)
